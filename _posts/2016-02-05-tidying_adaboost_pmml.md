...
1. The model corresponds to a classification problem, but is encoded as a regression-type [`MiningModel`](http://dmg.org/pmml/v4-3/MultipleModels.html) element. This may confuse PMML engines that employ different query and prediction workflows for different function types. For regression models, the query interface is typically no-op, and the prediction interface returns a single scalar value. Conversely, for classification models, the query interface exposes the universe of classes (eg. class labels and class descriptions), and the prediction interface returns the label of the winning class together with the class probability distribution. The AdaBoost PMML document aims to emulate the behaviour of conventional classification-type models, but does it rather poorly. For example, it completely lacks the the description of the target field (sic!), and class probabilities are calculated using the generic [`transformedValue`](http://dmg.org/pmml/v4-3/Output.html#ResFeat) output feature rather than the special-purpose [`probability`](http://dmg.org/pmml/v4-3/Output.html#ResFeat) output feature.
2. The `functionName` attribute of member [`TreeModel`](http://dmg.org/pmml/v4-3/TreeModel.html) elements has been specified as `regression`, but their encoding is consistent with classification-type models instead. Again, this kind of false signalling may confuse standards-compliant PMML engines.
3. Bloated [`MiningSchema`](http://dmg.org/pmml/v4-3/MiningSchema.html) elements. The idea of ensemble models is that every member model deals with a well-defined subspace of the input space. Unfortunately, the legacy `pmml` package is incapable of filtering out irrelevant active fields, and populates all `MiningSchema` elements with exactly the same set of 72 [`MiningField`](http://dmg.org/pmml/v4-3/MiningSchema.html) elements. It isn't just a matter of style. For example, it prevents from conducting custom variable importance analyses using simple XQuery/XPath language queries.
4. Bloated predicate elements. By default, the legacy `pmml` package encodes splits as [`CompoundPredicate`](http://dmg.org/pmml/v4-3/TreeModel.html#xsdElement_CompoundPredicate) elements, even though [`SimplePredicate`](http://dmg.org/pmml/v4-3/TreeModel.html#xsdElement_SimplePredicate) elements would suffice. Digging deeper, there are problems with the `CompoundPredicate` element itself, because it does not convey the functional difference between the "main" split and "alternative" splits.

##### Simplifying Predicate elements

The behaviour of the [`rpart()`](https://www.rdocumentation.org/packages/rpart/versions/4.1-13/topics/rpart) function, which underlies the [`ada()`](https://www.rdocumentation.org/packages/ada/versions/2.0-5/topics/ada) function, is controlled using options. The default configuration is aimed at being applicable to most diverse use cases. For example, it supports missing values in the training dataset, and generates a set of "alternative" splits in addition to the "main" split.
...
The options can be passed to the `ada()` function as an [`rpart.control`](https://www.rdocumentation.org/packages/rpart/versions/4.1-13/topics/rpart.control) object. Use `maxsurrogate = 0` and `maxcompete = 0` for disabling surrogate splits and "alternative" splits, respectively.
...
The removal of `ScoreDistribution` elements can be easily undone. The idea is to score a dataset and capture the identifiers of winning Nodes using the [`entityId`](http://dmg.org/pmml/v4-3/Output.html#ResFeat) output feature.
