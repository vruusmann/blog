...
The [`GeneralRegressionModel`](http://dmg.org/pmml/v4-3/GeneralRegression.html) element is more flexible (eg. contrast matrices, parameterizable link functions), but is encoded in a matrix-oriented way that is rather difficult to parse and follow for humans.
The [`RegressionModel`](http://dmg.org/pmml/v4-3/Regression.html) elements loses in functionality but makes it up in human-friendliness.
